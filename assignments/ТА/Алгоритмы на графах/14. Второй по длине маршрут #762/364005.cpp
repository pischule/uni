...
struct qd_comp {
  bool operator()(const qd& a, const qd& b) { return a.dist > b.dist; }
};
...
path_with_length dijkstra(int start, int end, int k) {
  ...
  for (auto& v : distances) v.reserve(k);
  ...
  while (!q.empty() && distances[end].size() < k) {
    ...
    for (edge& e : adj_lst[curr.to]) {
      ...
      if (distances[e.to].size() < k) q.push({curr.to, vc, alt_dist, e.to});
    }
  }
...
int main() {
  ...
  path_with_length pwl = dijkstra(start - 1, end - 1, 2);
  fout << pwl.length << '\n';
  for (int i : pwl.path) fout << i + 1 << ' ';
