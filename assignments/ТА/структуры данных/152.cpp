...
struct Point {
    Pos pos;
    int land_height;
    Point(Pos pos, int land_height)
        :pos(pos), land_height(land_height)
    {}

    Point(int y, int x, int land_height)
            :pos{y, x}, land_height(land_height)
    {}
};
...
long f() {
    ...
    while (!down_queue.empty()) {
        ...
        for (Pos dir: directions) {
            ...
            if (is_legal(n_p) && visited[n_p.y][n_p.x] && (form[curr.y][curr.x] + water_on_top[curr.y][curr.x])
